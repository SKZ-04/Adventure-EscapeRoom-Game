...
public class AdventureLoader {
    ...
    private void parseRooms() throws IOException {
        ...
        String roomFileName = this.adventureName + "/newRooms.txt";
    ...
    public void parseObjects() throws IOException {
        ...
        while (buff.ready()) {
            ...
            String objectCombatBool = buff.readLine();
            ...
            boolean combatItem = Boolean.parseBoolean(objectCombatBool);
            AdventureObject object = new AdventureObject(objectName, objectDescription, combatItem, location);
            location.addGameObject(object);
        }

    }
    public void parseEnemies() throws IOException {
        String enemyFileName = this.adventureName + "/enemies.txt";
        BufferedReader buff = new BufferedReader(new FileReader(enemyFileName));

        while(buff.ready()) {
            String enemyName = buff.readLine();
            String enemyHealth = buff.readLine();
            String enemyLocation = buff.readLine();
            String separator = buff.readLine();
            if (separator != null && !separator.isEmpty())
                System.out.println("Formatting Error!");
            int i = Integer.parseInt(enemyLocation);
            int h = Integer.parseInt(enemyHealth);
            Room location = this.game.getRooms().get(i);
            Enemy enemy = new Enemy(h, enemyName);
